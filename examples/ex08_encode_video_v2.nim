...
proc main =
  var outFileName = "media/deleteme.mp4"
  var codecName   = "libx264"
  echo paramCount()
  if paramCount() != 2:
...
For example:
  ex08_encode_video.nim media/deleteme.mp4 libx264
    ...
    codecName = paramStr(2)

  # Find codec by name
  let codec = findEncoderByName(codecName)

  # Given the codec, we allocate a context and configure it
  ...
  if codec.handle.id in @[AV_CODEC_ID_MPEG1VIDEO, AV_CODEC_ID_MPEG2VIDEO]:
